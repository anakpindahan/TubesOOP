...
GameState::GameState() {
	...
	// Membuat Player
	map[player.getCoordinate().getY()][player.getCoordinate().getX()].setEntity(
			"Player");
	map[player.getCoordinate().getY()][player.getCoordinate().getX()].setSymbol(
			'P');
	...
	// Mengatur engimon yang aktif

	for (int i = 1; i <= playerEngimon; i++) {
		cout << "Nomor Urut: " << i << endl;
		player.getInventoryEngimon().getEngimon(i).showStat();
		cout << endl;
	}
	cout << "Silahkan pilih nomor engimon yang ingin dijadikan active engimon: " << endl;
	int idActive;
	cin >> idActive;
	player.setActiveEngimon(idActive);
	player.setEngimonCoordinate(0, 1);
	map[1][0].setSymbol('A');
	map[1][0].setEntity("AEng");
...
void GameState::movePlayer() {
	...
	if (valid(k, l)) {
		if (valid(map[l][k])) {
			// Active engimon bergerak duluan
			map[player.getEngimonCoordinate().getY()][player.getEngimonCoordinate().getX()].setEntity("Air");
			map[player.getEngimonCoordinate().getY()][player.getEngimonCoordinate().getX()].setNaturalSymbol();
			player.setEngimonCoordinate(i, j);
			player.setPrevCoordinate(i, j);
			map[j][i].setEntity("AEng");
			map[j][i].setSymbol('A');
...
void GameState::executeCommand(string _command) {
	if (_command == "Move") {
		...
		cout << "Silahkan pilih ingin ditukar dengan engimon nomor berapa" << endl;
		int i;
		cin >> i;
	...
	while (j > 0 && i < MAX_WILD) {
		...
		if (wildEngimons[i].getId() <= 0) {
			...
			wildEngimons[i] = RealEngimon(e.getId(), WILD_ENGIMON, e.getSpecies(),
					e.getSlogan(), e.getMaxExp(), Cell(), 20 + (rand() % 10),
					rand() % 100, "No Name");
			for(int k = 0; k < e.getNumElements(); k++){
